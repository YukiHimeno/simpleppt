...
function renderCover(plan: PagePlan, style: SlideStyle, l: BentoLayout): string {
  const titleLines = wrapLines(plan.title, style.carded ? 56 : 54, l.W - l.PAD * 2 - 200, 2)
  const parts: string[] = []
  const kickerY = l.CONTENT_TOP + 30
  parts.push(T(l.PAD, kickerY, 13, plan.kicker || 'SIMPLEPPT', { fill: style.accent, weight: 600, ls: 3 }))
  if (style.carded) {
    ...
    parts.push(questionPerson(l.W - l.PAD, l.CONTENT_TOP + 60, 54))
  }
  const titleSize = style.carded ? 56 : 54
  let ty = kickerY + 92
  ...
  parts.push(T(l.PAD, ty + 16, 17, plan.message, { fill: style.muted }))
