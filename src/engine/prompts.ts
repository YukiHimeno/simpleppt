...
- 相邻句同款：不要连续几句用同一个句子骨架（"A 完成了……。B 完成了……。C 完成了……。"），打散句法，长短错落。
- 破折号装饰：不拿"——"做停顿揭晓或插入补充，用逗号、句号直接写完整。
- 冒号空转：不写"一句话总结：""核心是：""原因如下：""常见做法包括："这类冒号引出空内容的句子，内容直接写出来。
- 起手填充："说白了""说穿了""先说结论""值得注意的是""值得一提的是"这类开场直接删掉。
- 拟人化职业喻体：不要把系统、流程比作"智慧的导师""贴身的数字秘书""永不疲倦的审查员"这类理想化的人，写它实际做了什么。
- 概括词盖数据：有具体数字就写数字，不用"显著提升""大幅增长"盖住"从 60 万降到 42 万"这类实值。
- 翻译腔壳子：少用"对于……来说""就……而言""当……时""此外""然而""这意味着"等句首壳，把对象放进句子里，连接词放句中或换成"不过""其实""也"。
- 编号骨架：不要用"一、二、三"或"第一、第二、第三"给小标题通篇编号。
...
- 标题位置不固定，按权重自选一种：
  - 标题在左上角（默认、最常见）：约 45%
  - 本页省略独立标题：约 30%（标题的含义并入正文/要点首句，不要另起一行再写一遍标题）
  - 标题顶部居中：约 10%
  - 标题竖排在页面左侧（文字旋转 -90° 纵向排布，不占正文宽度）：约 10%
  - 标题居中放在页面中上部作大字视觉：约 5%
...
- cover：左对齐构图。${style.carded ? `accent 短横线 rect(x=${l.PAD},y=252,w=64,h=6,rx=3)；kicker 13px accent letter-spacing 3 在 (${l.PAD}, 226)；主标题 56px 字重 700 fg，基线 (${l.PAD}, 330)（超过 14 字拆两行，第二行基线 +72）；副标题 17px muted 一行，基线 (${l.PAD}, 386)；底部 meta (${l.PAD}, 640) 13px muted。右上象限一组克制的几何装饰：2 个同心圆环 stroke="accent" opacity 0.25-0.4 + 1 个小实心方块，不与文字重叠。` : `kicker 13px accent letter-spacing 3 在 (${l.PAD}, ${l.CONTENT_TOP + 30})；主标题用 Office 艺术字样式（font-weight=800、fill="rgba(255,255,0,0.42)"、stroke="#FFC000" stroke-width=2 paint-order="stroke"）60px，基线 (${l.PAD}, ${l.CONTENT_TOP + 96})（超 14 字拆两行）；**不画任何强调色短横线**；副标题 18px muted 一行，基线 +64；底部 meta 13px muted (${l.PAD}, ${l.FOOTER_Y - 40})；右下角放一个"问号小人"占位：<g data-question-person="1" x="1024" y="150" size="60"/>（不要画矢量轮廓）。重点词可用荧光笔。`}
